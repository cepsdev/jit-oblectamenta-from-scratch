...
void create_ret_fragment_and_execute(bool debug_info = true){
    ...
    auto loc{0};
    loc += opcode_mov_dest_reg_source_imm(code_frag + loc, registers::RSI, 1);
    loc += opcode_mov_dest_reg_source_imm(code_frag + loc, registers::RDI, 2);
    loc += opcode_add_dest_reg_source_reg(code_frag + loc, registers::RSI,registers::RDI);
